...
<div class="px-4 flex justify-center">
    <div class="flex justify-center w-full border max-w-3xl hover:shadow-none relative shadow-lg bg-gray-400">
        ...
        <div class="buttons flex absolute font-bold right-0 text-xs text-gray-500 space-x-0 my-3.5 mr-3">
            <button class="bg-white add border rounded-l-2xl rounded-r-sm border-gray-300 p-1 px-4 cursor-pointer hover:bg-gray-700 hover:text-white" onclick="follow({{ $user->id }})">フォロー</button>
...
<div class="px-4 flex justify-center">
    <ul class="flex flex-wrap justify-center text-sm font-medium text-center text-gray-500 dark:text-gray-400 grid grid-cols-2 px-16 py-4 max-w-4xl w-full">
        <li class="flex justify-center nav-item col-span-1  border-b-2 border-white">
            <a href="{{ route('posthome',$user->id) }}" class="nav-link inline-block text-gray-400 active text-xl py-3">
                投稿
            </a>
        </li>
        <li class="flex justify-center nav-item col-span-1 bg-white border-b-2 border-white">
            <a href="{{ route('othersbookmarks',$user->id) }}" class="nav-link inline-block text-muted text-xl py-3 text-muted active text-2xl">
                お気に入り
            </a>
        </li>
    </ul>
